...
### 动态链接
参考自[文章](https://time.geekbang.org/column/article/437653)
两个大问题：公共库函数的代码要在多个不同的进程中进行共享，如何处理其地址问题；符号链接/重定位过程时怎样的。

- `LIBRARY_PATH` 针对(编译时)链接器，一般系统默认是 `gnu ld`。失败时会产生`/usr/bin/ld: cannot find -lfoo`的错误。其可通过 `-L` 等价指定路径
- `LD_LIBRARY_PATH` 针对(运行时)动态链接器，即我们通过 `ldd` 看到的 `ld-linux-x86-64.so.2` 这个库。失败时会长生`./A: error while loading shared libraries: libfoo.so: cannot open shared object file: No such file or directory`的错误。

可执行文件或者动态库文件被加载进内存的时候，文件中不同的 section 会被加载进内存中不同 segment，
e.g. `.data` 和`.bss` 段被加载进数据段 (`data segment`)，而`.code`, `.rodata` 被加载进代码段 (`code segment`)。

所以，最终效果是，动态库的数据段和代码段是靠在一起的，但和可执行程序的数据段、代码段相独立，由此可以通过虚拟地址技术，实现动态库代码段共享，且不同进程映射的同一段代码段地址可以不同。

此时，对变量、函数等的相对寻址方法不再可行。相对寻址要求目标地址和本条指令的地址之间的相对值是固定的，但此时不同进程可能会产生不同的映射值，所以引入`地址无关代码`。思路是通过添加一个`中间层`，使得对全局符号的访问由`直接访问`变成`间接访问`。

地址无关代码 (`Position Independent Code`, `PIC`)技术通过引入一个中间层作为固定地址来间接查找函数地址，即`全局偏移表` (`Global Offset Table`, `GOT`)： 其中存放的是该模块需要访问的所有外部符号的地址。这样可以使得对外部符号的访问转换为对 GOT 表的访问。由于 GOT 表的相对偏移在同一个 so 中肯定是不变的，所以对 GOT 的访问可以使用相对寻址完成。GOT 中指向的是调用目标的在各自进程中的虚拟地址，我们是通过 GOT 表间接访问的方式，将对外部符号地址的直接依赖消除了。这些外部符号都需要进行解析和重定位。这个工作由 loader 负责，其为符号分配并记录地址，然后将这些地址回写进 GOT 表。

地址无关的代码除了可以在 so 中使用，同样可以在可执行文件中使用，可以通过 -pie 选项使得 gcc 编译地址无关的可执行文件。地址无关的可执行文件可以被加载到内存的任意位置执行，这会使得缓冲区溢出的难度增加（你可以结合第 4 节课思考一下原因），但代价是通过 GOT 访问地址会多一次访存，性能会下降。

总结：
1. 静态库实它可以看成是中间文件的简单集合，保留了符号，只有在静态链接的时候才会真正地做地址分配和重定位，并与可执行文件打包链接在一起
2. 动态库在编译阶段时其代码并不会整合进可执行文件中，只在运行时加载进内存，且地址不固定，所以每次加载完成以后，才能为它地符号分配真实地内存地址，然后再把地址回填到引用它的GOT中。其有点是可以在多个进程间共享，从而减少内存的重复。